...
def main():
    ...
    youtube_video_url = st.text_input("🖋️ Please write down the YouTube address. example: https://www.youtube.com/watch?v=t9IDoenf-lo",placeholder="https://www.youtube.com/watch?v=**********")
    st.markdown('---')
    ...
    if len(youtube_video_url) > 2:
        ...
        # URL을 제대로 입력했을 경우
            ...
            result = chain.invoke({"input_documents": text})
            # print(result)
            summarized_text = result["output_text"]

            st.session_state["summarize"] = summarized_text
